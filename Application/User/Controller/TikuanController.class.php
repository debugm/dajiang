...
class TikuanController extends UserController
{
    ...
    public function sqjs()
    {
        ...
        $apilist = $Payapi->where(array('is_tx' => 1))->select();
	...
	$jstype = M('Payapi')->where("id=".$tongdaoid)->getField("jstype");
	//T1
	$today_m = 0;
	if($jstype == 1)
	{
		// 需计算当天跑了多少量，结算金额要去除当天量	
		
		$tongdao = M('Payapi')->where("id=".$tongdaoid)->getField("en_payname");
		$starttime = mktime(0,0,0,date('m'),date('d'),date('Y'));
		$endtime = mktime(0,0,0,date('m'),date('d')+1,date('Y')) - 1;
		$where = array();
		$where['pay_memberid'] = session("userid") + 10000;
		$where['pay_tongdao'] = $tongdao;
		$where['pay_successdate'] = array('between',array(($starttime),($endtime)));
		
		$today_m = M('Order')->where($where)->sum('pay_amount');
	}

        $yuemoney = $Apimoney->where("userid=" . session("userid") . " and payapiid=" . $tongdaoid)->getField("money") - $today_m;
