...
class UserController extends AdminController
{
    ...
    public function usertji()
    {
		...
		$endtime = mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;	
		$cond = array();
		$cond['pay_successdate'] = array('between',array(($starttime),($endtime)));
		
		//$tjlist = M("Order")->where($cond)->getField("pay_memberid,sum(pay_amount)")->group("pay_memberid")->select();
		$tjlist = M("Order")->query("SELECT pay_memberid,pay_tongdao,sum(pay_amount) FROM `pay_order` WHERE pay_successdate > {$starttime} and pay_successdate < {$endtime} group by pay_tongdao,pay_memberid");
		$td = array();
		foreach($tdlist as $item)
		{
		    $td[$item['en_payname']] = 0;
		}	
		$res = array();
		foreach($tjlist as $item)
		{

		    
		    $temp = array();
		    if(!array_key_exists($item['pay_memberid'],$res))
		    {
		        $temp = $td;
		        $temp[$item['pay_tongdao']] = $item["sum(pay_amount)"];
		        $res[$item['pay_memberid']] = $temp;
		    }
	            else
		    {
			$res[$item['pay_memberid']][$item['pay_tongdao']] = $item["sum(pay_amount)"];
		    }
		}
