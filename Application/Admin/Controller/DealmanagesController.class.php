...
class DealmanagesController extends AdminController
{
	...
	$memberid = I("request.userid",0,'intval');
        if ($memberid) {
 //           $where['userid'] = array('eq',$memberid);
	...
	$tjdate_ks = urldecode(I("request.tjdateks"));
        ...
        if ($tjdate_ks || $tjdate_js) {
            $where['oidtime'] = array('between',array(strtotime($tjdate_ks),strtotime($tjdate_js)?strtotime($tjdate_js):time()));
        }
