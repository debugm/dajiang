...
class XmzfbwapController extends PayController
{
    //商户私钥
    ...
    public function Pay(){
        ...
        $parameter = array(
            'PayName' => 'Xmzfbwap', // 通道名称
            'zh_PayName' => 'Zfbwap-Xm',
            'moneyratio' => 1, // 金额比例
            'tjurl' => '',
            'orderid' => $orderid,
            'body'=>$body,
...
header("Location:".$url);
    ...
    {	
        file_put_contents("./log_esd.txt.xm",date('Y-m-d H:i:s')."notify-----".json_encode($_GET)."\n",FILE_APPEND);
	...
	{
        	echo 'opstate=0';	
        	$this->EditMoney2($oid, 'Xmzfbwap', 0,1);
	}
    }
