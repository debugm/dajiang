...
class QtqqController extends PayController
{
    //商户私钥
    ...
    public function Pay(){
        ...
        $notifyurl = $this->_site . 'Pay_Qtqq_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Qtqq_callbackurl.html'; //返回通知
        $parameter = array(
            'PayName' => 'Qtqq', // 通道名称
            'zh_PayName' => 'QQ扫码',
            'moneyratio' => 1, // 金额比例
		...
		$acclist = M('Userbankaccount')->where("userid=".$memberid." and enable=1 and bankcode=".$return['bankcode'])->select();
		...
		/*
                ...
                if($arr['respCode'] == "0")
                {
                    ...
                    $this->assign("imgurl", $arr['codeUrl']);
    ...
    public function notifyurl()
    {
        ...
        $this->EditMoney($order_no, 'Qtqq', 0);
    }
