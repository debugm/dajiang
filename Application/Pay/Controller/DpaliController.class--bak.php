...
class DpaliController extends PayController
{
    //商户私钥
    ...
    public function Pay(){
        ...
        $notifyurl = $this->_site . 'Pay_Dpali_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Dpali_callbackurl.html'; //返回通知
        $parameter = array(
            'PayName' => 'Dpali', // 通道名称
            'zh_PayName' => '智付支付宝扫码',
            'moneyratio' => 1, // 金额比例
        ...
        if ($return["status"] == "error") {
            ...
            $service_type = 'alipay_scan';//微信：weixin_scan 支付宝：alipay_scan 智汇宝：zhb_scan
    ...
    public function callbackurl()
    {
        ...
        if($pay_status <> 0) {
            $this->EditMoney($_REQUEST['orderid'], 'Dpali', 1);
            echo "success";
        }
    ...
    public function notifyurl()
    {
        ...
        //响应"SUCCESS"
        if($flag){
            $this->EditMoney($order_no, 'Dpali', 0);
            echo"SUCCESS";
        }else{
