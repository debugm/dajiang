...
class DpbankController extends PayController
{
    ...
    public function Pay(){
        ...
        $notifyurl = 'http://pay.yunshi44.top/Pay_Dpbank_notifyurl.html'; //异步通知
        $callbackurl = 'http://pay.yunshi44.top/Pay_Dpbank_callbackurl.html'; //返回通知
        $parameter = array(
            'PayName' => 'Dpbank', // 通道名称
            'zh_PayName' => '智付网银',
            'moneyratio' => 1, // 金额比例
	 ...
	 // 获取用户的通道账户进行轮询
		...
		$bankcode = $return["bankcode"];
                $acclist = M('Userbankaccount')->where("userid=$memberid and enable=1 and bankcode='{$bankcode}'")->select();
    ...
    {
        file_put_contents('d3.txt',serialize($_REQUEST));
        ...
        if($flag==true){
            $Order = M("Order");
            $pay_status = $Order->where("pay_orderid = '".$order_no."'")->getField("pay_status");
            if($pay_status <> 0) {
                $this->EditMoney($order_no, 'Dpbank', 1);
                echo "success";
            }
        }else{
            echo "deposit failed";
    ...
    {
        file_put_contents('d2.txt',serialize($_REQUEST));
        $merchant_code	= $_POST["merchant_code"];
        $interface_version = $_POST["interface_version"];
        $sign_type = $_POST["sign_type"];
        $dinpaySign = base64_decode($_POST["sign"]);
