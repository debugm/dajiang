...
class DpwxController extends PayController
{
    //商户私钥
    ...
    public function Pay(){
        ...
        $notifyurl = $this->_site . 'Pay_Dpwx_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Dpwx_callbackurl.html'; //返回通知
        $parameter = array(
            'PayName' => 'Dpwx', // 通道名称
            'zh_PayName' => '智付微信扫码',
            'moneyratio' => 1, // 金额比例
        ...
        if ($return["status"] == "error") {
            ...
            $service_type = 'weixin_scan';//微信：weixin_scan 支付宝：alipay_scan 智汇宝：zhb_scan
    ...
    public function callbackurl()
    {
        ...
        if($pay_status <> 0) {
            $this->EditMoney($_REQUEST['orderid'], 'Dpwx', 1);
            echo "success";
        }
    ...
    public function notifyurl()
    {
        ...
        //响应"SUCCESS"
        if($flag){
            $this->EditMoney($order_no, 'Dpwx', 0);
            echo"SUCCESS";
        }else{
