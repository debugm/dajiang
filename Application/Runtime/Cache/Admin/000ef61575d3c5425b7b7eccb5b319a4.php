...
<div class="tab-content">
  ...
  <!-------------------------------------------------------------------------------------------------->
  <form  role="form" id="tikuanconfigform" action="<?php echo U("Tikuan/Tikuanconfigedit");?>" class="form-horizontal">
   <input type="hidden" id="tkconfigid" name="tkconfigid" value="<?php echo ($tikuanconfiglist["id"]); ?>">
  ...
  <div class="form-group">
      <label>是否开通T+1：</label>
       ...
       <script>
	   $("#t1zt").val('<?php echo ($tikuanconfiglist["t1zt"]); ?>');
	   </script>
  </div>
   <div class="form-group">
      <label>是否开通T+0：</label>
       <select class="form-control" name="t0zt" id="t0zt">
        ...
        <script>
	   $("#t0zt").val('<?php echo ($tikuanconfiglist["t0zt"]); ?>');
	   </script>
  </div>
   <div class="form-group">
  <label>购买T+0金额：</label>
  ...
  <div class="form-group">
      <label>提款状态：</label>
       <select class="form-control" name="tkzt" id="tkzt">
        ...
        <script>
	   $("#tkzt").val('<?php echo ($tikuanconfiglist["tkzt"]); ?>');
	   </script>
  </div>
  ...
  <div class="tab-pane" id="tksjsz">
    <!------------------------------------------------------------------------------->
    <form role="form" action="<?php echo U("Tikuan/tikuanshijianedit");?>" class="form-horizontal">
      <fieldset>
        <div class="form-group">
          <label>白天提款时间</label>
        </div>
  ...
  <div class="tab-pane" id="tkjjrsz">
        ...
        <div class="inputcontent"> 请选择排除的日期：<input type="text"  id="pcjjrval" name="pcjjrval"  class="laydate-icon" onclick="laydate({ele:this,max: laydate.now(+1),istime: false,
            istoday: true,format: 'YYYY-MM-DD'})" style="height: 30px; width:160px;">&nbsp;&nbsp;<button type="button" id="buttonpcjjr" class="layui-btn layui-btn-small" ajaxurl="<?php echo U("Tikuan/pcjjradd");?>">添加</button></div>
        <input type="hidden" id="pcjjrdelurl" value="<?php echo U("Tikuan/pcjjrdel");?>">
        <div class="sjcontent" id="pcjjr">
            <?php if(is_array($pcjjrlist)): $i = 0; $__LIST__ = $pcjjrlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div><?php echo ($vo["datetime"]); ?> <button class="layui-btn layui-btn-danger layui-btn-small" onclick="javascript:pcjjrdel('<?php echo ($vo["id"]); ?>',this)">删除</button></div><?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
        <div style="clear:left;"></div>
        ...
        <button type="button" id="buttontjjjr" class="layui-btn layui-btn-small" ajaxurl="<?php echo U("Tikuan/tjjjradd");?>">添加</button></div>
         <input type="hidden" id="tjjjrdelurl" value="<?php echo U("Tikuan/tjjjrdel");?>">
        <div class="sjcontent" id="tjjjr">
        	 <?php if(is_array($tjjjrlist)): $i = 0; $__LIST__ = $tjjjrlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div><?php echo ($vo["datetime"]); ?>(<span><?php echo ($vo["shuoming"]); ?></span>) <button class="layui-btn layui-btn-danger layui-btn-small" onclick="javascript:tjjjrdel('<?php echo ($vo["id"]); ?>',this)">删除</button></div><?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
  </div>
</div>
